...
```bash
curl -sSL https://raw.githubusercontent.com/JiLiZART/mtproto-installer/main/install.sh | bash
```
...
- Домен маскировки по умолчанию: `ya.ru`. Интерактивно можно ввести другой; без TTY: `FAKE_DOMAIN=sberbank.ru curl -sSL ... | bash`.
...
После `git clone https://github.com/JiLiZART/mtproto-installer.git && cd mtproto-installer` можно запустить `./install.sh` — скрипт по умолчанию качает файлы с того же GitHub. Либо настроить вручную и поднять без скрипта:
...
```bash
curl -sSL https://raw.githubusercontent.com/JiLiZART/mtproto-installer/main/uninstall.sh | bash
```
